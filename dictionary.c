...
bool check(const char *word)
{
    ...
    //compare words making sure it is case insensitive
    if (strcasecmp(cursor->word, word) == 0)
    {
        return true;
    }

    //keep traversing the linked list until it finds the word or it finishes
    while (cursor->next != NULL)
    {
        //check each word along the way
        cursor = cursor->next;
        if (strcasecmp(cursor->word, word) == 0)
        {
            return true;
...
//assign a number for each letter of the alphabet
...
bool load(const char *dictionary)
{
    ...
    // opens the dictionary and allocates memory to hold the words
    ...
    // reads file until the end of file mark
